...
import { Order, OrderStatus, AbandonedCart } from '@/src/types';
import { ShoppingBag, Edit, Heart, TrendingUp, MessageCircle, Clock, ShoppingCart, AlertCircle } from 'lucide-react';
            ...
            {metrics && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-brand-blue/30 transition-colors">
                        ...
                        <p className="text-xs text-slate-500 mt-1">Pedidos Enviados/Recebidos.</p>
                    </div>
                            ...
                            {filteredOrders.map(order => {
                                ...
                                return (
                                    <tr key={order.id} className="hover:bg-slate-800/30">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${order.status === 'PAID' ? 'bg-green-900/30 text-green-400 border-green-500/30' :
