...
export const FlixManager: React.FC = () => {
    ...
    useEffect(() => { setProfiles(mockService.getFlixProfiles()); }, [viewMode]);
    ...
    const handleSave = () => {
        if (!editForm.slug || !editForm.displayName) { alert("Slug e Nome são obrigatórios."); return; }
        const cleanSlug = editForm.slug.toLowerCase().replace(/[^a-z0-9-]/g, '-');
        const newProfile = { ...editForm, slug: cleanSlug, linkedOrderId: checkOrderId || undefined } as FlixProfile;
        mockService.saveFlixProfile(newProfile);
